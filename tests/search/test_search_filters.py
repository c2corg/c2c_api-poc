...
from c2corg_api.search.search_filters import create_filter, build_query, create_bbox_filter
...
from elasticsearch_dsl.query import Range, Term, Terms, Bool, GeoBoundingBox, Missing
...
class AdvancedSearchTest(BaseTestCase):
    ...
    def assertFiltersEqual(self, filters1, filters2):  # noqa
        ...
        normal_filters1 = set(str(f) for f in filters1 if "geo_bounding_box" not in f)
        normal_filters2 = set(str(f) for f in filters2 if "geo_bounding_box" not in f)
    ...
    def test_create_bbox_filter(self):
        ...
        self.assertEqual(create_bbox_filter("650000,4500000,650000,5700000"), None)
        self.assertEqual(create_bbox_filter("500000,5700000,650000,5700000"), None)
        self.assertEqual(create_bbox_filter("650000,5700000,650000,5700000"), None)
    ...
    def assertBboxFilterEqual(self, f1, f2):  # noqa
        ...
        self.assertEqual(f1["geo_bounding_box"].get("type"), f2["geo_bounding_box"].get("type"))
