...
class TestTopoMapRest(BaseDocumentTestRest):
    ...
    def test_get_collection_paginated(self):
        ...
        self.assertResultsEqual(self.get_collection({"offset": 0, "limit": 1}), [self.map4.document_id], 4)
        self.assertResultsEqual(
            self.get_collection({"offset": 0, "limit": 2}), [self.map4.document_id, self.map3.document_id], 4
        )
        self.assertResultsEqual(
            self.get_collection({"offset": 1, "limit": 2}), [self.map3.document_id, self.map2.document_id], 4
        )
    ...
    def test_get_collection_search(self):
        ...
        self.assertResultsEqual(
            self.get_collection_search({"l": "en"}), [self.map4.document_id, self.map1.document_id], 2
        )
    ...
    def test_put_success_figures_only(self):
        ...
        (body, map1) = self.put_success_figures_only(body, self.map1, user="moderator")
    ...
    def test_put_success_lang_only(self):
        ...
        (body, map1) = self.put_success_lang_only(body, self.map1, user="moderator")

        self.assertEqual(map1.get_locale("en").title, "New title")
    ...
    def test_put_success_new_lang(self):
        ...
        (body, map1) = self.put_success_new_lang(body, self.map1, user="moderator")
    ...
    def _add_test_data(self):
        ...
        self.session.add(TopoMapAssociation(document=self.waypoint2, topo_map=self.map1))
        self.session.flush()
