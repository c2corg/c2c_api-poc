...
from c2corg_api.models.feed import DocumentChange, update_feed_document_create, update_feed_images_upload
from c2corg_api.models.image import Image, ArchiveImage
from c2corg_api.models.outing import Outing, OutingLocale, ArchiveOuting, ArchiveOutingLocale
from c2corg_api.models.route import Route, RouteLocale, ArchiveRoute, ArchiveRouteLocale
from c2corg_api.models.topo_map import TopoMap
from c2corg_api.models.topo_map_association import TopoMapAssociation
from c2corg_api.models.waypoint import Waypoint, WaypointLocale, ArchiveWaypoint, ArchiveWaypointLocale
from c2corg_api.models.xreport import Xreport, XreportLocale, ArchiveXreport, ArchiveXreportLocale
...
class TestDocumentDeleteRest(BaseTestRest):
    def setUp(self):  # noqa
        ...
        self.waypoint4.locales.append(WaypointLocale(lang="en", title="Mont Ventoux", description="..."))
        ...
        self.outing1 = Outing(
            ...
            locales=[
                OutingLocale(
                    lang="en", title="...", description="...", weather="sunny", document_topic=DocumentTopic(topic_id=6)
                )
            ],
        )
        ...
        self.outing2 = Outing(
            ...
            locales=[
                OutingLocale(
                    lang="en", title="...", description="...", weather="sunny", document_topic=DocumentTopic(topic_id=7)
                )
            ],
        )
        ...
        self.session.flush()
        DocumentRest.update_version(self.article1, user_id, "new title", [UpdateType.LANG], [article1_lang])
        self.session.flush()
        ...
        self.article2 = Article(
            ...
            categories=["gear"],
            article_type="personal",
            locales=[DocumentLocale(lang="en", title="Some other article", description="Some content")],
        )
        ...
        self.book1 = Book(
            ...
            locales=[
                DocumentLocale(
                    lang="en", title="Some book", description="Some content", document_topic=DocumentTopic(topic_id=9)
                )
            ],
        )
        ...
        self.session.flush()
        DocumentRest.update_version(self.image1, user_id, "changed filename", [UpdateType.FIGURES], [])
        self.session.flush()
        ...
        self.session.flush()
        self.session.add(TopoMapAssociation(document=self.waypoint2, topo_map=self.topo_map1))
        self.session.add(TopoMapAssociation(document=self.waypoint3, topo_map=self.topo_map1))
        self.session.add(TopoMapAssociation(document=self.route2, topo_map=self.topo_map1))
        self.session.add(TopoMapAssociation(document=self.route3, topo_map=self.topo_map1))
        self.session.flush()
    ...
    def _add_association(self, parent_document, child_document):
        association = Association.create(parent_document=parent_document, child_document=child_document)
        self.session.add(association)
        self.session.add(association.get_log(self.global_userids["contributor"]))

    def _add_tag(self, document):
        document_id = document.document_id
        document_type = document.type
        user_id = self.global_userids["contributor"]
        tag = DocumentTag(document_id=document_id, document_type=document_type, user_id=user_id)
        self.session.add(tag)
        log = DocumentTagLog(document_id=document_id, document_type=document_type, user_id=user_id, is_creation=True)
        self.session.add(log)

    def _delete(self, document_id, expected_status):
        headers = self.add_authorization_header(username="moderator")
        return self.app.delete_json(self._prefix + str(document_id), headers=headers, status=expected_status)

    def test_non_unauthorized(self):
        self.app.delete_json(self._prefix + str(self.waypoint1.document_id), {}, status=403)
    ...
    def test_delete_main_waypoint(self):
        ...
        self.assertErrorsContain(
            response.json, "document_id", "This waypoint cannot be deleted because it is a main waypoint."
        )
    ...
    def test_delete_former_main_waypoint(self):
        ...
        self.session.flush()
        DocumentRest.update_version(self.route3, self.global_userids["contributor"], "Update", [UpdateType.FIGURES], [])
    ...
    def test_delete_only_waypoint_of_route(self):
        ...
        self.assertErrorsContain(
            response.json,
            "document_id",
            "This waypoint cannot be deleted because " "it is the only waypoint associated to some routes.",
        )
    ...
    def test_delete_only_route_of_outing(self):
        ...
        self.assertErrorsContain(
            response.json,
            "document_id",
            "This route cannot be deleted because " "it is the only route associated to some outings.",
        )

    def _test_delete(
        self, document_id, clazz, clazz_locale, archive_clazz, archive_clazz_locale, expected_deleted_docs_count=1
    ):
        ...
        self.assertEqual(initial_doc_count - count, expected_deleted_docs_count)
        ...
        count = self.session.query(clazz).filter(getattr(clazz, "document_id") == document_id).count()
        self.assertEqual(0, count)
        count = self.session.query(Document).filter(Document.document_id == document_id).count()
        self.assertEqual(0, count)
        if clazz_locale:
            count = self.session.query(clazz_locale).filter(getattr(clazz_locale, "document_id") == document_id).count()
            self.assertEqual(0, count)
        count = self.session.query(DocumentLocale).filter(DocumentLocale.document_id == document_id).count()
        self.assertEqual(0, count)
        count = self.session.query(DocumentGeometry).filter(DocumentGeometry.document_id == document_id).count()
        self.assertEqual(0, count)

        # Check the archives have been cleared too
        count = self.session.query(archive_clazz).filter(getattr(archive_clazz, "document_id") == document_id).count()
        self.assertEqual(0, count)
        count = self.session.query(ArchiveDocument).filter(ArchiveDocument.document_id == document_id).count()
        self.assertEqual(0, count)
        if archive_clazz_locale:
            ...
            self.assertEqual(0, count)
        count = (
            self.session.query(ArchiveDocumentLocale).filter(ArchiveDocumentLocale.document_id == document_id).count()
        )
        self.assertEqual(0, count)
        count = (
        ...
        association_count = (
            ...
            .filter(or_(Association.parent_document_id == document_id, Association.child_document_id == document_id))
            .count()
        )
        ...
        association_log_count = (
            ...
            .filter(
                or_(AssociationLog.parent_document_id == document_id, AssociationLog.child_document_id == document_id)
            )
            .count()
        )
        self.assertEqual(0, association_log_count)
        association_count = (
            self.session.query(TopoMapAssociation).filter(TopoMapAssociation.document_id == document_id).count()
        )
        self.assertEqual(0, association_count)
        association_count = (
            self.session.query(AreaAssociation).filter(AreaAssociation.document_id == document_id).count()
        )
        self.assertEqual(0, association_count)

        # Check the feed has been cleared
        feed_count = self.session.query(DocumentChange).filter(DocumentChange.document_id == document_id).count()
        self.assertEqual(0, feed_count)

    def test_delete_waypoint(self):
        self._test_delete(self.waypoint3.document_id, Waypoint, WaypointLocale, ArchiveWaypoint, ArchiveWaypointLocale)
    ...
    def test_delete_route(self):
        self._test_delete(self.route3.document_id, Route, RouteLocale, ArchiveRoute, ArchiveRouteLocale)
    ...
    def _count_tags(self, document_id):
        nb_tags = self.session.query(DocumentTag).filter(DocumentTag.document_id == document_id).count()
        nb_logs = self.session.query(DocumentTagLog).filter(DocumentTagLog.document_id == document_id).count()
    ...
    def test_delete_outing(self):
        ...
        self._test_delete(self.outing1.document_id, Outing, OutingLocale, ArchiveOuting, ArchiveOutingLocale, 2)
    ...
    def test_delete_outing_route_waypoint(self):
        self._test_delete(self.outing2.document_id, Outing, OutingLocale, ArchiveOuting, ArchiveOutingLocale)
        ...
        self._test_delete(self.route2.document_id, Route, RouteLocale, ArchiveRoute, ArchiveRouteLocale)
        ...
        self._test_delete(self.waypoint2.document_id, Waypoint, WaypointLocale, ArchiveWaypoint, ArchiveWaypointLocale)

        # Check that associated documents cache versions have been incremented
        self.check_cache_version(self.route3.document_id, 3)

    def test_delete_article(self):
        self._test_delete(self.article1.document_id, Article, None, ArchiveArticle, None)
    ...
    def test_delete_xreport(self):
        self._test_delete(self.xreport1.document_id, Xreport, XreportLocale, ArchiveXreport, ArchiveXreportLocale)
    ...
    def test_delete_collaborative_doc(self):
        ...
        return self.app.delete_json(self._prefix + str(self.waypoint4.document_id), {}, headers=headers, status=400)

        # Moderators can:
        headers = self.add_authorization_header(username="moderator")
        return self.app.delete_json(self._prefix + str(self.waypoint4.document_id), {}, headers=headers, status=200)

    def test_delete_personal_doc_not_author(self):
        # Personal documents cannot be deleted by anyone:
        headers = self.add_authorization_header(username="contributor2")
        return self.app.delete_json(self._prefix + str(self.article1.document_id), {}, headers=headers, status=400)

        # Except by moderators:
        headers = self.add_authorization_header(username="moderator")
        return self.app.delete_json(self._prefix + str(self.article1.document_id), {}, headers=headers, status=200)
    ...
    def test_delete_personal_doc_author_more_24h(self):
        ...
        headers = self.add_authorization_header(username="contributor")
        return self.app.delete_json(self._prefix + str(self.article2.document_id), {}, headers=headers, status=400)

    def test_delete_locale_non_unauthorized(self):
        self.app.delete_json(self._prefix + str(self.waypoint1.document_id) + "/fr", {}, status=403)

        headers = self.add_authorization_header(username="contributor")
        return self.app.delete_json(
            self._prefix + str(self.waypoint1.document_id) + "/fr", {}, headers=headers, status=403
        )

    def _delete_locale(self, document_id, lang, expected_status):
        headers = self.add_authorization_header(username="moderator")
        return self.app.delete_json(
            self._prefix + str(document_id) + "/" + lang, headers=headers, status=expected_status
        )
    ...
    def _test_delete_locale(self, document_id, lang):
        ...
        initial_other_locale_archives_count = (
            ...
            .filter(ArchiveDocumentLocale.lang != lang)
            .count()
        )
        ...
        other_locale_archives_count = (
            self.session.query(ArchiveDocumentLocale)
            .filter(ArchiveDocumentLocale.document_id == document_id)
            .filter(ArchiveDocumentLocale.lang != lang)
            .count()
        )
        self.assertEqual(other_locale_archives_count, initial_other_locale_archives_count)
        ...
        self.assertEqual(other_locale_versions_count, initial_other_locale_versions_count)

        # Check the feed has been updated
        feed_items_langs = (
            self.session.query(DocumentChange.langs).filter(DocumentChange.document_id == document_id).all()
        )
