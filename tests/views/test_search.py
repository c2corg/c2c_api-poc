...
class TestSearchRest(BaseTestRest):
    ...
    def test_search_by_article_title(self):
        response = self.app.get(self._prefix + "?q=" + str(self.article1.locales[0].title), status=200)
        body = response.json
    ...
    def test_search_by_book_title(self):
        response = self.app.get(self._prefix + "?q=" + str(self.book1.locales[0].title), status=200)
        body = response.json
        books = body["books"]
    ...
    def test_search_authenticated(self):
        ...
        response = self.app.get(self._prefix + "?q=crolles", headers=headers, status=200)
        body = response.json
    ...
    def test_search_limit_types(self):
        response = self.app.get(self._prefix + "?q=crolles&t=w,r,c,b", status=200)
        body = response.json
    ...
    def test_search_by_document_id(self):
        response = self.app.get(self._prefix + "?q=" + str(self.waypoint1.document_id), status=200)
        body = response.json
