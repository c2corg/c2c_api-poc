...
class BaseTestRest(BaseTestCase):
    ...
    def post_json_with_contributor(self, url, body={}, status=200, username="contributor", headers={}):
    ...
    def assertNotNotifiedEs(self):  # noqa
        queue = self.queue_config.queue(self.queue_config.connection)
        self.assertIsNone(queue.get(), "unexpected sync. notification sent for ES")
    ...
    def get_feed_change(self, document_id, change_type=None):
        q = self.session.query(DocumentChange).filter(DocumentChange.document_id == document_id)
...
class BaseDocumentTestRest(BaseTestRest):
    def set_prefix_and_model(self, prefix, doc_type, model, model_archive, model_archive_locale):
        self._prefix = prefix
    ...
    def get_collection(self, params=None, user=None):
        ...
        if limit is None:
            if self._model == UserProfile:
                ...
                nb_docs = self.session.query(self._model).filter(getattr(self._model, "redirects_to").is_(None)).count()
    ...
    def get(self, reference, user=None, check_title=True, ignore_checks=False):
        headers = {} if not user else self.add_authorization_header(username=user)
        response = self.app.get(self._prefix + "/" + str(reference.document_id), headers=headers, status=200)
        self.assertEqual(response.content_type, "application/json")
    ...
    def get_caching(self, reference, user=None):
        ...
        cache_key = get_cache_key(reference.document_id, None, document_type=self._doc_type)
    ...
    def get_version(self, reference, reference_version, user=None):
        ...
        response = self.app.get(
            "{0}/{1}/en/{2}".format(self._prefix, str(reference.document_id), str(reference_version.id)),
            headers=headers,
            status=200,
        )
        self.assertEqual(response.content_type, "application/json")
        ...
        self.assertEqual(body["document"]["document_id"], reference.document_id)
        self.assertEqual(body["version"]["version_id"], reference_version.id)
    ...
    def get_info(self, reference, lang):
        response = self.app.get("{0}/{1}/{2}/info".format(self._prefix, str(reference.document_id), lang), status=200)
        self.assertEqual(response.content_type, "application/json")

        body = response.json
        self.assertIn("document_id", body)
        self.assertIn("locales", body)
        self.assertEqual(body["document_id"], reference.document_id)
    ...
    def _get_cooked(self, reference, lang, user=None):
        ...
        response = self.app.get(
            self._prefix + "/" + str(reference.document_id) + "?cook=" + lang, headers=headers, status=200
        )
        self.assertEqual(response.content_type, "application/json")
    ...
    def get_lang(self, reference, user=None):
        ...
        response = self.app.get(self._prefix + "/" + str(reference.document_id) + "?l=en", headers=headers, status=200)
        self.assertEqual(response.content_type, "application/json")
    ...
    def get_new_lang(self, reference, user=None):
        ...
        response = self.app.get(self._prefix + "/" + str(reference.document_id) + "?l=it", headers=headers, status=200)
        self.assertEqual(response.content_type, "application/json")
    ...
    def post_error(self, request_body, user="contributor"):
        ...
        response = self.app_post_json(self._prefix, request_body, headers=headers, expect_errors=True, status=400)
    ...
    def post_same_locale_twice(self, request_body):
        ...
        errors = body.get("errors")
        self.assertEqual(len(errors), 1)
        self.assertEqual(errors[0].get("description"), 'lang "en" is given twice')
        self.assertEqual(errors[0].get("name"), "locales")
        return body
    ...
    def post_non_whitelisted_attribute(self, request_body, user="contributor"):
        ...
        headers = self.add_authorization_header(username=user)
        response = self.app_post_json(self._prefix, request_body, headers=headers, status=200)
    ...
    def post_missing_content_type(self, request_body):
        response = self.app.post(self._prefix, params=json.dumps(request_body), status=415)
    ...
    def _validate_document(self, body, headers=None, validate_with_auth=False):
        ...
        if validate_with_auth:
            response = self.app.get(self._prefix + "/" + str(document_id), headers=headers, status=200)
        else:
            response = self.app.get(self._prefix + "/" + str(document_id), status=200)
        self.assertEqual(response.content_type, "application/json")
        ...
        self.sync_es()
        search_doc = search_documents[self._doc_type].get(id=doc.document_id, index=elasticsearch_config["index"])
    ...
    def put_wrong_document_id(self, request_body, user="contributor"):
        ...
        headers = self.add_authorization_header(username=user)
        response = self.app_put_json(self._prefix + "/9999999", request_body, headers=headers, status=404)
    ...
    def put_wrong_version(self, request_body, id, user="contributor"):
        response = self.app_put_json(self._prefix + "/" + str(id), request_body, status=403)

        headers = self.add_authorization_header(username=user)
        response = self.app_put_json(self._prefix + "/" + str(id), request_body, headers=headers, status=409)
    ...
    def put_wrong_ids(self, request_body, id, user="contributor"):
        ...
        request body.
        """
        response = self.app_put_json(self._prefix + "/" + str(id + 1), request_body, status=403)

        headers = self.add_authorization_header(username=user)
        response = self.app_put_json(self._prefix + "/" + str(id + 1), request_body, headers=headers, status=400)
        body = response.json
        self.assertEqual(body["status"], "error")
    ...
    def put_put_no_document(self, id, user="contributor"):
        ...
        headers = self.add_authorization_header(username=user)
        response = self.app_put_json(self._prefix + "/" + str(id), request_body, headers=headers, status=400)

        body = response.json
        self.assertEqual(body["status"], "error")
        self.assertEqual(body["errors"][0]["description"], "Required")

    def put_missing_field(self, request_body, document, field, user="contributor"):
        response = self.app_put_json(self._prefix + "/" + str(document.document_id), request_body, status=403)

        headers = self.add_authorization_header(username=user)
        response = self.app_put_json(
            self._prefix + "/" + str(document.document_id), request_body, headers=headers, status=400
        )
    ...
    def put_success_all(self, request_body, document, user="contributor", check_es=True, cache_version=2):
        ...
        headers = self.add_authorization_header(username=user)
        self.app_put_json(self._prefix + "/" + str(document.document_id), request_body, headers=headers, status=200)

        response = self.app.get(self._prefix + "/" + str(document.document_id), headers=headers, status=200)
        self.assertEqual(response.content_type, "application/json")

        body = response.json
        document_id = body.get("document_id")
        self.assertNotEqual(body.get("version"), document.version)
        ...
        self.assertEqual(archive_document_en.version, document.version)
        ...
        self.assertEqual(archive_locale_fr.version, self.locale_fr.version)
    ...
    def put_success_figures_only(self, request_body, document, user="contributor", check_es=True):
        ...
        # check updates to the search index
        sync_es(self.session)
        search_doc = search_documents[self._doc_type].get(id=document.document_id, index=elasticsearch_config["index"])
        ...
        if isinstance(document, Route) and document.main_waypoint_id:
            ...
            self.assertEqual(search_doc["title_en"], document.get_locale("en").title)
            self.assertEqual(search_doc["title_fr"], document.get_locale("fr").title)
    ...
    def put_success_new_lang(self, request_body, document, user="contributor", check_es=True):
        ...
        if check_es:
            ...
            self.assertEqual(search_doc["title_en"], document.get_locale("en").title)
            self.assertEqual(search_doc["title_fr"], document.get_locale("fr").title)
            self.assertEqual(search_doc["title_es"], document.get_locale("es").title)
    ...
    def _get_association_logs(self, document):
        ...
        self.assertNotEqual(len(associations), 0, "Need at least one association")
