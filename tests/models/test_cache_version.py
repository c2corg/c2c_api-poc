...
class TestCacheVersion(BaseTestCase):
    ...
    def test_trigger_create_cache_version(self):
        ...
        cache_version = self.session.query(CacheVersion).get(waypoint.document_id)
    ...
    def test_update_cache_version_single_wp(self):
        ...
        cache_version_untouched = self.session.query(CacheVersion).get(waypoint_unrelated.document_id)
    ...
    def test_update_cache_version_wp_with_associations(self):
        ...
        waypoint_unrelated = Waypoint(waypoint_type="summit")
        self.session.add_all([waypoint1, waypoint2, waypoint3, waypoint_unrelated])
        self.session.flush()
        ...
        cache_version1 = self.session.query(CacheVersion).get(waypoint1.document_id)
        cache_version2 = self.session.query(CacheVersion).get(waypoint1.document_id)
        cache_version3 = self.session.query(CacheVersion).get(waypoint1.document_id)
        cache_version_untouched = self.session.query(CacheVersion).get(waypoint_unrelated.document_id)
    ...
    def test_update_cache_version_wp_as_main_wp(self):
        ...
        self.session.add_all([waypoint1, waypoint2, waypoint3, waypoint_unrelated, route])
        self.session.flush()
        ...
        update_cache_version(waypoint1)
        cache_version_wp1 = self.session.query(CacheVersion).get(waypoint1.document_id)
        cache_version_wp2 = self.session.query(CacheVersion).get(waypoint2.document_id)
        cache_version_wp3 = self.session.query(CacheVersion).get(waypoint3.document_id)
        cache_version_route = self.session.query(CacheVersion).get(route.document_id)
        cache_version_untouched = self.session.query(CacheVersion).get(waypoint_unrelated.document_id)
    ...
    def test_update_cache_version_route(self):
        ...
        waypoint2 = Waypoint(waypoint_type="summit")
        waypoint_unrelated = Waypoint(waypoint_type="summit")
        self.session.add_all([waypoint1, waypoint2, waypoint_unrelated, route1, route2])
        self.session.flush()
        ...
        cache_version_route1 = self.session.query(CacheVersion).get(route1.document_id)
        cache_version_route2 = self.session.query(CacheVersion).get(route2.document_id)
        cache_version_wp1 = self.session.query(CacheVersion).get(waypoint1.document_id)
        cache_version_wp2 = self.session.query(CacheVersion).get(waypoint2.document_id)
        cache_version_untouched = self.session.query(CacheVersion).get(waypoint_unrelated.document_id)
    ...
    def test_update_cache_version_outing(self):
        outing = Outing(
            activities=["skitouring"], date_start=datetime.date(2016, 2, 1), date_end=datetime.date(2016, 2, 1)
        )
        route1 = Route(activities=["skitouring"])
        route2 = Route(activities=["skitouring"])
        waypoint1 = Waypoint(waypoint_type="summit")
        waypoint2 = Waypoint(waypoint_type="summit")
        waypoint_unrelated = Waypoint(waypoint_type="summit")
        self.session.add_all([outing, waypoint1, waypoint2, waypoint_unrelated, route1, route2])
        self.session.flush()
        ...
        cache_version_outing = self.session.query(CacheVersion).get(outing.document_id)
        cache_version_route1 = self.session.query(CacheVersion).get(route1.document_id)
        cache_version_route2 = self.session.query(CacheVersion).get(route2.document_id)
        cache_version_wp1 = self.session.query(CacheVersion).get(waypoint1.document_id)
        cache_version_wp2 = self.session.query(CacheVersion).get(waypoint2.document_id)
    ...
    def test_update_cache_version_user(self):
        ...
        cache_version_user_profile = self.session.query(CacheVersion).get(user_profile.document_id)
        cache_version_outing = self.session.query(CacheVersion).get(outing.document_id)
    ...
    def test_update_cache_version_user_document_version(self):
        ...
        """
        waypoint = Waypoint(
            waypoint_type="summit", elevation=2203, locales=[WaypointLocale(lang="en", title="...", description="...")]
        )
        ...
        user = User(
            name="test_user",
            username="test_user",
            email="test_user@camptocamp.org",
            forum_username="testuser",
            password="test_user",
        ...
        update_cache_version(user_profile)
        cache_version_user_profile = self.session.query(CacheVersion).get(user_profile.document_id)
        cache_version_waypoint = self.session.query(CacheVersion).get(waypoint.document_id)
    ...
    def test_update_cache_version_associations_removed_wp(self):
        ...
        cache_version1 = self.session.query(CacheVersion).get(waypoint1.document_id)
        cache_version2 = self.session.query(CacheVersion).get(waypoint2.document_id)
        cache_version3 = self.session.query(CacheVersion).get(waypoint3.document_id)
        cache_version_untouched = self.session.query(CacheVersion).get(waypoint_unrelated.document_id)
    ...
    def test_update_cache_version_associations_removed_route_outing(self):
        ...
        self.session.add_all([waypoint1, waypoint2, waypoint3, waypoint_unrelated, route, outing])
        self.session.flush()
    ...
    def test_update_cache_version_for_area(self):
        ...
        self.session.add(AreaAssociation(document_id=waypoint.document_id, area_id=area.document_id))
        self.session.flush()
        ...
        cache_version_waypoint = self.session.query(CacheVersion).get(waypoint.document_id)
        cache_version_untouched = self.session.query(CacheVersion).get(waypoint_unrelated.document_id)
        cache_version_area = self.session.query(CacheVersion).get(area.document_id)
    ...
    def test_update_cache_version_for_map(self):
        ...
        self.session.add(TopoMapAssociation(document_id=waypoint.document_id, topo_map_id=topo_map.document_id))
        self.session.flush()
        ...
        cache_version_waypoint = self.session.query(CacheVersion).get(waypoint.document_id)
        cache_version_untouched = self.session.query(CacheVersion).get(waypoint_unrelated.document_id)
        cache_version_map = self.session.query(CacheVersion).get(topo_map.document_id)
