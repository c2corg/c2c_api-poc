...
class SyncTest(BaseTestCase):
    ...
    def test_get_changed_documents(self):
        ...
        self.assertEqual(len(get_changed_documents(self.session, last_update + datetime.timedelta(0, 1))), 0)

    def test_get_changed_documents_for_associations(self):
        last_update, _ = es_sync.get_status(self.session)

        changed_documents = get_changed_documents_for_associations(self.session, last_update)
        ...
        self.assertEqual(
            len(get_changed_documents_for_associations(self.session, last_update + datetime.timedelta(0, 1))), 0
        )
    ...
    def test_get_deleted_locale_documents(self):
        ...
        self.session.add(ESDeletedLocale(document_id=self.waypoint1.document_id, type="w", lang="fr"))
        self.session.flush()
    ...
    def test_sync_documents(self, mock):
        ...
        self.assertEqual(set(route_doc["waypoints"]), {self.waypoint1.document_id, self.waypoint2.document_id})

        outing_doc = self._get_by_id(search_documents, self.outing1.document_id)
        self.assertEqual(outing_doc["title_en"], "...")
        self.assertEqual(set(outing_doc["waypoints"]), {self.waypoint1.document_id, self.waypoint2.document_id})
    ...
    def _get_by_id(self, search_documents, document_id):
        return next(filter(lambda doc: doc["_id"] == document_id, search_documents), None)
    ...
    def _add_test_data(self):
        ...
        self.waypoint1 = Waypoint(
            ...
            locales=[
                ...
                WaypointLocale(lang="en", title="Mont Granier", description="...", summary="The Mont Granier"),
            ],
        )
        ...
        self.route1 = Route(
            ...
            durations="1",
            locales=[
                RouteLocale(lang="en", title="Face N", description="...", gear="paraglider", title_prefix="Mont Blanc")
            ],
        )
        ...
        self.session.add_all([self.waypoint1, self.waypoint2, self.waypoint3, self.route1, self.outing1])
        self.session.flush()
        ...
        user = self.session.query(UserProfile).get(self.global_userids["contributor"])
