...
class MergeUsersTest(BaseTestCase):
    ...
    def test_merge_user_accounts(self):
        ...
        count = self.session.query(UserProfile).filter(UserProfile.document_id == source_id).count()
        self.assertEqual(0, count)
        count = self.session.query(Document).filter(Document.document_id == source_id).count()
        self.assertEqual(0, count)
        count = self.session.query(DocumentLocale).filter(DocumentLocale.document_id == source_id).count()
        self.assertEqual(0, count)
        count = self.session.query(DocumentGeometry).filter(DocumentGeometry.document_id == source_id).count()
        self.assertEqual(0, count)

        # Check the archives have been cleared too
        count = self.session.query(ArchiveUserProfile).filter(ArchiveUserProfile.document_id == source_id).count()
        self.assertEqual(0, count)
        count = self.session.query(ArchiveDocument).filter(ArchiveDocument.document_id == source_id).count()
        self.assertEqual(0, count)
        count = self.session.query(ArchiveDocumentLocale).filter(ArchiveDocumentLocale.document_id == source_id).count()
        self.assertEqual(0, count)
        count = (
            self.session.query(ArchiveDocumentGeometry).filter(ArchiveDocumentGeometry.document_id == source_id).count()
        )
        self.assertEqual(0, count)
        ...
        count = self.session.query(SsoExternalId).filter(SsoExternalId.user_id == source_id).count()
        self.assertEqual(0, count)
    ...
    def _count_tags(self, user_id):
        return self.session.query(DocumentTag).filter(DocumentTag.user_id == user_id).count()

    def _count_tag_logs(self, user_id):
        return self.session.query(DocumentTagLog).filter(DocumentTagLog.user_id == user_id).count()

    def _count_area_associations(self, user_id):
        return self.session.query(AreaAssociation).filter(AreaAssociation.document_id == user_id).count()

    def _count_associations(self, user_id):
        return (
            self.session.query(Association)
            .filter(or_(Association.parent_document_id == user_id, Association.child_document_id == user_id))
            .count()
        )
    ...
    def _count_feed_entries(self, user_id):
        return self.session.query(DocumentChange).filter(DocumentChange.user_id == user_id).count()

    def _count_associated_feed_entries(self, user_id):
        return self.session.query(DocumentChange).filter(any_(DocumentChange.user_ids) == user_id).count()

    def _count_metadata_entries(self, user_id):
        return self.session.query(HistoryMetaData).filter(HistoryMetaData.user_id == user_id).count()

    def _is_registered_to_ml(self, user):
        return self.session.query(exists().where(Mailinglist.email == user.email)).scalar()
    ...
    def _add_test_data(self):
        ...
        ml = Mailinglist(listname="avalanche", user_id=self.contributor.id, email=self.contributor.email)
        self.session.add(ml)
        ...
        self.session.flush()
        self.session.add(TopoMapAssociation(document=self.contributor.profile, topo_map=self.map1))
        self.session.flush()
        ...
        self.session.add(Association.create(parent_document=self.waypoint1, child_document=self.route1))
        ...
        self.outing1 = Outing(
            ...
            geometry=DocumentGeometry(geom_detail="SRID=3857;LINESTRING(635956 5723604, 635966 5723644)"),  # noqa
        )
        ...
        self.session.add(
            DocumentTag(document_id=self.route1.document_id, document_type=ROUTE_TYPE, user_id=self.contributor.id)
        )
        self.session.add(
            DocumentTagLog(
