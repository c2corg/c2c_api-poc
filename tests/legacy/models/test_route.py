...
from c2corg_api.ext.colander_ext import wkbelement_from_geojson, geojson_from_wkbelement
...
class TestRoute(BaseTestCase):
    ...
    def test_geometry_update(self):
        ...
        geom1 = wkbelement_from_geojson(
            json.loads(
                '{"type": "LineString", "coordinates": ' + "[[635956, 5723604, 1200], [635966, 5723644, 1210]]}"
            ),
            3857,
        )
        route_db = Route(
            document_id=1, activities=["hiking"], geometry=DocumentGeometry(document_id=1, geom=None, geom_detail=geom1)
        )

        geom2 = wkbelement_from_geojson(
            json.loads(
                '{"type": "LineString", "coordinates": ' + "[[635956, 5723604, 9999], [635966, 5723644, 9999]]}"
            ),
            3857,
        )
        route_in = Route(document_id=1, activities=["hiking"], geometry=DocumentGeometry(geom=None, geom_detail=geom2))
        route_db.update(route_in)
        self.assertIs(route_db.geometry.geom_detail, geom1)

        geom3 = wkbelement_from_geojson(
            json.loads(
                '{"type": "LineString", "coordinates": ' + "[[635956, 5723608, 1200], [635966, 5723644, 1210]]}"
            ),
            3857,
        )
        route_in = Route(document_id=1, activities=["hiking"], geometry=DocumentGeometry(geom=None, geom_detail=geom3))
        route_db.update(route_in)
    ...
    def test_simplify(self):
        geom = wkbelement_from_geojson(
            json.loads(
                '{"type": "LineString", "coordinates": ' + "[[635900, 5723600], [635902, 5723600], [635905, 5723600]]}"
            ),
            3857,
        )
        route = Route(activities=["hiking"], geometry=DocumentGeometry(geom=None, geom_detail=geom))
        ...
        route.geometry.geom_detail = wkbelement_from_geojson(
            json.loads(
                '{"type": "LineString", "coordinates": ' + "[[635901, 5723600], [635902, 5723600], [635905, 5723600]]}"
            ),
            3857,
        )
        ...
        geojson = json.loads(geojson_from_wkbelement(simplified_geom.geom_detail))
        self.assertEqual(len(geojson["coordinates"]), 2)
        self.assertCoodinateEquals([635901, 5723600], geojson["coordinates"][0])
