...
from c2corg_api.models.topo_map_association import update_map, TopoMapAssociation, update_maps_for_document, get_maps
...
class TestTopoMapAssociation(BaseTestCase):
    def setUp(self):  # noqa
        ...
        self.map1 = TopoMap(
            ...
            geometry=DocumentGeometry(
                geom_detail="SRID=3857;POLYGON((668518.249382151 5728802.39591739,668518.249382151 5745465.66808356,689156.247019149 5745465.66808356,689156.247019149 5728802.39591739,668518.249382151 5728802.39591739))"
            ),  # noqa
        )
        ...
        self.waypoint4 = Waypoint(waypoint_type="summit", geometry=DocumentGeometry())
        self.session.add_all([self.map2, self.waypoint1, self.waypoint2, self.waypoint3, self.waypoint4])
        self.session.flush()
        ...
        self.waypoint5 = Waypoint(
            ...
            geometry=DocumentGeometry(geom="SRID=3857;POINT(677461.381691516 5740879.44638645)"),
        )
    ...
    def test_update_map(self):
        ...
        added_links = (
            self.session.query(TopoMapAssociation).filter(TopoMapAssociation.topo_map_id == self.map1.document_id).all()
        )
        self.assertEqual(len(added_links), 1)
        self.assertEqual(added_links[0].document_id, self.waypoint1.document_id)

    def test_update_map_reset_first(self):
        # add an association with wp2, which should be removed after the update
        self.session.add(TopoMapAssociation(document_id=self.waypoint2.document_id, topo_map_id=self.map1.document_id))
        self.session.flush()
        ...
        self.assertEqual(updated_links[0].document_id, self.waypoint1.document_id)

    def test_update_map_reset_first_forwarded(self):
        """Tests that maps with `redirects_to` are ignored."""
        # add an association with wp2, which should be removed after the update
        self.session.add(TopoMapAssociation(document_id=self.waypoint2.document_id, topo_map_id=self.map2.document_id))
        self.session.flush()

        update_map(self.map2, reset=True)

        updated_links = (
            self.session.query(TopoMapAssociation).filter(TopoMapAssociation.topo_map_id == self.map2.document_id).all()
        )
    ...
    def test_update_document(self):
        ...
        self.assertEqual(len(added_links), 1)
        self.assertEqual(added_links[0].topo_map_id, self.map1.document_id)
    ...
    def test_get_maps(self):
        ...
        self.assertEqual(maps[0].document_id, self.map1.document_id)

    def test_get_maps_forwarded(self):
        """Tests that forwarded maps are not included."""
        # add an association with wp1
        self.session.add(TopoMapAssociation(document_id=self.waypoint1.document_id, topo_map_id=self.map2.document_id))
        self.session.flush()
    ...
    def _get_links_for_document(self, doc):
        return self.session.query(TopoMapAssociation).filter(TopoMapAssociation.document_id == doc.document_id).all()
