...
class RateLimitingTest(BaseTestRest):
    def setUp(self):  # noqa
        ...
        self.limit_moderator = int(self.settings["rate_limiting.limit_moderator"])
    ...
    def _test_requests(self):
        limit = self.limit_robot if self.user.robot else self.limit_moderator if self.user.moderator else self.limit
        ...
        delta = (
            datetime.datetime.now(pytz.utc) + datetime.timedelta(seconds=self.window_span) - self.user.ratelimit_reset
        )
    ...
    def _create_document(self):
        body = {
            ...
            "locales": [{"lang": "en", "title": "Mont Granier"}],
        }
        headers = self.add_authorization_header(username=self.username)
        response = self.app_post_json(self._prefix, body, headers=headers, status=200)
    ...
    def _update_document(self, status=200):
        ...
        body = {
            ...
            "document": {
                ...
                "locales": [{"version": self.document["locales"][0]["version"], "lang": "en", "title": "Mont Granier"}],
            },
        }
        headers = self.add_authorization_header(username=self.username)
        self.app_put_json(self._prefix + "/" + str(document_id), body, headers=headers, status=status)
        self._set_document(document_id)
    ...
    def _delete_document(self):
        headers = self.add_authorization_header(username=self.username)
        self.app.delete_json("/documents/delete/" + str(self.document["document_id"]), headers=headers, status=200)

    def _wait(self):
        waiting_time = self.window_span + 1
        print("Waiting %d secs the rate limiting window expires..." % waiting_time)
