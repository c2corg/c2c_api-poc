        ...
        for field in fields:
            ...
            if isinstance(mapping_field, QueryableMixin):
                if hasattr(mapping_field, "_model_field"):
                    self.assertTrue(mapping_field._model_field is getattr(model, field))
                if hasattr(mapping_field, "_enum_range"):
                    self.assertFalse(
                        mapping_field._enum_range, "Field {0} should be listed as ENUM_RANGE_FIELDS".format(field)
                    )

        enum_range_fields = search_model.ENUM_RANGE_FIELDS if hasattr(search_model, "ENUM_RANGE_FIELDS") else []
