...
from c2corg_api.caching import cache_document_listing, cache_document_history, cache_document_version
...
from c2corg_api.models.waypoint import Waypoint, WaypointLocale, ArchiveWaypoint, ArchiveWaypointLocale, WAYPOINT_TYPE
from c2corg_api.models.document import DocumentGeometry, ArchiveDocumentGeometry, DocumentLocale, DOCUMENT_TYPE
...
class TestWaypointRest(BaseDocumentTestRest):
    ...
    def test_get(self):
        ...
        self.assertEqual(self.waypoint4.document_id, linked_waypoints[0].get("document_id"))
        ...
        self.assertEqual(linked_route["document_id"], self.route1.document_id)
        self.assertEqual(linked_route.get("locales")[0].get("title_prefix"), "Mont Blanc :")
        self.assertEqual(linked_routes[0]["document_id"], self.route3.document_id)
    ...
    def test_get_with_empty_arrays(self):
        ...
        response = self.app.get(self._prefix + "/" + str(self.waypoint2.document_id), status=200)
        body = response.json
        self.assertIn("rock_types", body)
        self.assertEqual(body["rock_types"], [])

    def test_get_edit(self):
        response = self.app.get(self._prefix + "/" + str(self.waypoint.document_id) + "?e=1", status=200)
        body = response.json
    ...
    def test_get_redirected_wp(self):
        response = self.app.get(self._prefix + "/" + str(self.waypoint5.document_id), status=200)
        body = response.json
    ...
    def test_get_etag(self):
        response = self.app.get(self._prefix + "/" + str(self.waypoint.document_id), status=200)
        ...
        # then request the document again with the etag
        headers = {"If-None-Match": etag}
        response = self.app.get(self._prefix + "/" + str(self.waypoint.document_id), status=304, headers=headers)
    ...
    def test_get_cache_down(self):
        ...
        with detail_cache_mock, listings_cache_mock:
            self.app.get(self._prefix + "/" + str(self.waypoint.document_id), status=200)
    ...
    def test_get_info_redirect(self):
        response = self.app.get(self._prefix + "/" + str(self.waypoint5.document_id) + "/en/info", status=200)
        body = response.json
    ...
    def test_post_success(self):
        ...
        self.assertEqual(archive_geometry.document_id, doc.geometry.document_id)
    ...
    def test_put_success_all(self):
        ...
        self.assertEqual(search_doc["title_en"], "Mont Granier! : Mont Blanc from the air")
    ...
    def test_put_add_geometry(self):
        ...
        response = self.app.get(self._prefix + "/" + str(waypoint.document_id), status=200)
        self.assertEqual(response.content_type, "application/json")

        body = response.json
        document_id = body.get("document_id")
        self.assertEqual(body.get("version"), body_put.get("document").get("version"))
    ...
    def test_put_merged_wp(self):
        ...
        self.assertEqual(len(errors), 1)
        self.assertEqual(errors[0].get("description"), "can not update merged document")
    ...
    def test_put_protected_no_permission(self):
        ...
        errors = response.json.get("errors")
        self.assertEqual(len(errors), 1)
        self.assertEqual(errors[0].get("description"), "No permission to change a protected document")
    ...
    def test_history_etag(self):
        ...
        # then request the document again with the etag
        headers = {"If-None-Match": etag}
        self.app.get("/document/%d/history/%s" % (id, "fr"), status=304, headers=headers)
    ...
    def test_history_caching(self):
        ...
        # check that the response is cached
        self.app.get("/document/%d/history/%s" % (waypoint_id, "fr"), status=200)
        ...
        response = self.app.get("/document/%d/history/%s" % (waypoint_id, "fr"), status=200)
        body = response.json
        self.assertEqual(body, fake_cache_value)
    ...
    def test_get_documents_no_version(self):
        ...
        self.assertEqual(documents[0]["document_id"], self.waypoint.document_id)
        self.assertEqual(documents[1]["document_id"], self.waypoint2.document_id)
    ...
    def _add_test_data(self):
        ...
        self.session.add(TopoMapAssociation(document=self.waypoint, topo_map=self.topo_map2))
        self.session.flush()
