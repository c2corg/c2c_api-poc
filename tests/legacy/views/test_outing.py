...
from c2corg_api.models.outing import Outing, ArchiveOuting, ArchiveOutingLocale, OutingLocale, OUTING_TYPE
...
class TestOutingRest(BaseDocumentTestRest):
    ...
    def test_get_collection_for_route(self):
        ...
        response = self.app.get(self._prefix + "?r=" + str(self.route.document_id), status=200)
    ...
    def test_get_collection_for_user(self):
        reset_search_index(self.session)
        response = self.app.get(self._prefix + "?u=" + str(self.global_userids["contributor"]), status=200)
    ...
    def test_get(self):
        ...
        self.assertEqual(body.get("activities"), self.outing.activities)
        ...
        self.assertEqual(self.route.document_id, linked_routes[0].get("document_id"))

        linked_users = associations.get("users")
        self.assertEqual(len(linked_users), 1)
        # TODO id -> document_id
        self.assertEqual(linked_users[0]["document_id"], self.global_userids["contributor"])

        linked_images = associations.get("images")
        self.assertEqual(len(linked_images), 1)
        self.assertEqual(linked_images[0]["document_id"], self.image.document_id)
    ...
    def test_get_edit(self):
        response = self.app.get(self._prefix + "/" + str(self.outing.document_id) + "?e=1", status=200)
        body = response.json
    ...
    def test_get_sort_asc(self):
        ...
        reset_search_index(self.session)
        response = self.app.get(self._prefix + "?sort=height_diff_up", status=200)
    ...
    def test_get_sort_desc(self):
        ...
        reset_search_index(self.session)
        response = self.app.get(self._prefix + "?sort=-elevation_max", status=200)
        response_ids = [d["document_id"] for d in response.json["documents"]]
    ...
    def test_get_sort_multi(self):
        ...
        height_diff_up: asc)"""
        reset_search_index(self.session)
        response = self.app.get(self._prefix + "?sort=-elevation_max,height_diff_up", status=200)
        response_ids = [d["document_id"] for d in response.json["documents"]]
        outing_ids = [d.document_id for d in [self.outing2, self.outing, self.outing4, self.outing3]]
        self.assertEqual(response_ids, outing_ids)
    ...
    def test_get_sort_numeric_enum(self):
        ...
        reset_search_index(self.session)
        response = self.app.get(self._prefix + "?sort=-elevation_access,condition_rating", status=200)
        response_ids = [d["document_id"] for d in response.json["documents"]]
    ...
    def test_get_sort_error(self):
        ...
        """
        reset_search_index(self.session)
        self.app.get(self._prefix + "?sort=-elevation_axess", status=500)
    ...
    def test_post_empty_activities_error(self):
        ...
        self.assertEqual(error.get("description"), "Shorter than minimum length 1")
    ...
    def test_put_success_all(self):
        ...
        self.assertEqual(archive_document_en.activities, ["skitouring", "hiking"])
    ...
    def test_put_success_association_update(self):
        ...
        response = self.app.get(self._prefix + "/" + str(self.outing.document_id), headers=headers, status=200)
