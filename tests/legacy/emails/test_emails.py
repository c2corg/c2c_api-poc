...
class EmailTests(BaseTestCase):
    ...
    def test_send_email(self, smtp):
        ...
        smtp.assert_called_once_with(self.settings["mail.host"], port=self.settings["mail.port"])

        smtp.return_value.sendmail.assert_called_once_with(self.settings["mail.from"], "toto@localhost", ANY)
