...
#     is_moderator = DBSession.query(User).filter(User.id == userid, User.moderator).count() > 0
#     return ["group:moderators"] if is_moderator else [Authenticated]


# def is_valid_token(token):
#     now = datetime.datetime.utcnow()
#     token = DBSession.query(Token).filter(Token.value == token, Token.expire > now).first()

#     if not token:
#         return False
#     else:
#         user_is_blocked = DBSession.query(User.blocked).filter(User.id == token.userid).scalar()
#         if user_is_blocked:
...
#     token = DBSession.query(Token).filter(Token.value == value, User.id == userid).first()
#     if not token:
