...
from c2corg_api.models.area_association import update_area, AreaAssociation, update_areas_for_document, get_areas
...
class TestAreaAssociation(BaseTestCase):
    def setUp(self):  # noqa
        ...
        self.session.add_all([self.area2, self.waypoint1, self.waypoint2, self.waypoint3, self.waypoint4])
        self.session.flush()
    ...
    def test_update_area(self):
        ...
        added_links = (
            self.session.query(AreaAssociation).filter(AreaAssociation.area_id == self.area1.document_id).all()
        )
    ...
    def test_update_area_reset_first(self):
        ...
        self.session.add(AreaAssociation(document_id=self.waypoint2.document_id, area_id=self.area1.document_id))
        self.session.flush()
    ...
    def test_update_area_reset_first_forwarded(self):
        ...
        # add an association with wp2, which should be removed after the update
        self.session.add(AreaAssociation(document_id=self.waypoint2.document_id, area_id=self.area2.document_id))
        self.session.flush()

        update_area(self.area2, reset=True)

        updated_links = (
            self.session.query(AreaAssociation).filter(AreaAssociation.area_id == self.area2.document_id).all()
        )
    ...
    def test_update_document(self):
        ...
        self.assertEqual(len(added_links), 1)
        self.assertEqual(added_links[0].area_id, self.area1.document_id)
    ...
    def test_get_areas_forwarded(self):
        ...
        self.session.add(AreaAssociation(document_id=self.waypoint1.document_id, area_id=self.area2.document_id))
        self.session.flush()
        areas = get_areas(self.waypoint1, "en")
    ...
    def _get_links_for_document(self, doc):
        return self.session.query(AreaAssociation).filter(AreaAssociation.document_id == doc.document_id).all()
