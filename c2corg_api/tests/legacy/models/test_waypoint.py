...
from c2corg_api.models.document import UpdateType, DocumentGeometry, set_available_langs
...
class TestWaypoint(BaseTestCase):
    ...
    def test_version_is_incremented(self):
        waypoint = Waypoint(
            waypoint_type="summit", elevation=2203, locales=[WaypointLocale(lang="en", title="A", description="abc")]
        )
        self.session.add(waypoint)
        self.session.flush()
    ...
    def test_geometry_update_optimization_4326(self):
        ...
        waypoint_in = Waypoint(
            ...
            waypoint_type="summit",
            elevation=1234,
            geometry=DocumentGeometry(geom="SRID=4326;POINT(" + str(lat + 10 ** (-8)) + " 6.0)"),
        )
        ...
        waypoint_in = Waypoint(
            ...
            waypoint_type="summit",
            elevation=1234,
            geometry=DocumentGeometry(geom="SRID=4326;POINT(" + str(lat + 10 ** (-7)) + " 6.0)"),
        )
        waypoint_db.update(waypoint_in)
    ...
    def test_save_geometry(self):
        waypoint = self._get_waypoint()
        waypoint.geometry = DocumentGeometry(geom="SRID=3857;POINT(635956.075332665 5723604.677994)")
        self.session.add(waypoint)
        self.session.flush()
    ...
    def test_get_geometry_lon_lat(self):
        ...
        self.assertAlmostEqual(lon_lat_geojson["coordinates"][0], 5.7128906, places=6)
        self.assertAlmostEqual(lon_lat_geojson["coordinates"][1], 45.644768, places=6)
    ...
    def test_array_handling_non_empty(self):
        waypoint = Waypoint(waypoint_type="summit", elevation=2203, rock_types=["basalte"])
        self.session.add(waypoint)
        self.session.flush()
    ...
    def _get_waypoint(self):
        return Waypoint(
            ...
            geometry=DocumentGeometry(geom="SRID=3857;POINT(635956.075332665 5723604.677994)"),
        )
