...
class TestDocumentChange(BaseTestCase):
    def setUp(self):  # noqa
        ...
        self.user1 = self.session.query(User).get(self.global_userids["contributor"])
        self.user2 = self.session.query(User).get(self.global_userids["contributor2"])
    ...
    def test_check_changes_on_area_deletion_ok(self):
        ...
        self.session.query(CacheVersion).filter(CacheVersion.document_id == self.area1.document_id).delete()
