...
class TestAssociation(BaseTestCase):
    ...
    def _add_test_data_routes(self):
        self.session.add_all(
            [
                Association.create(parent_document=self.route1, child_document=self.route2),
                Association.create(parent_document=self.waypoint1, child_document=self.route1),
                Association.create(parent_document=self.waypoint2, child_document=self.route1),
            ]
        )
        self.session.flush()
    ...
    def test_get_current_associations_routes(self):
        ...
        new_associations = {
            "routes": [{"document_id": 1, "is_parent": True}],
        ...
        expected_current_associations = {
            ...
            "waypoints": [
                ...
                {"document_id": self.waypoint2.document_id, "is_parent": True},
            ],
        }
    ...
    def test_get_current_associations_routes_partial(self):
        ...
        expected_current_associations = {"routes": [{"document_id": self.route2.document_id, "is_parent": False}]}
        self.assertEqual(current_associations, expected_current_associations)
    ...
    def test_get_current_associations_waypoints(self):
        ...
        self.session.add_all(
            [
                Association.create(parent_document=self.waypoint1, child_document=self.waypoint2),
                Association.create(parent_document=self.waypoint3, child_document=self.waypoint1),
                Association.create(parent_document=self.waypoint1, child_document=self.route1),
                Association.create(parent_document=self.waypoint1, child_document=self.route2),
            ]
        )
        self.session.flush()
        ...
        new_associations = {
            # routes are ignored
            "routes": [{"document_id": 1, "is_parent": True}, {"document_id": 2, "is_parent": True}],
            "waypoints": [{"document_id": 3, "is_parent": True}],
            "waypoint_children": [{"document_id": 4, "is_parent": False}],
        }

        current_associations = _get_current_associations(self.waypoint1, new_associations)

        expected_current_associations = {
            "waypoints": [{"document_id": self.waypoint3.document_id, "is_parent": True}],
    ...
    def test_get_current_associations_waypoints_partial(self):
        self.session.add(Association.create(parent_document=self.waypoint1, child_document=self.waypoint2))
        self.session.flush()
    ...
    def test_diff_associations(self):
        new_associations = {
            "routes": [{"document_id": 1, "is_parent": True}, {"document_id": 2, "is_parent": True}],
            "waypoints": [{"document_id": 3, "is_parent": False}],
        }
        current_associations = {
            "routes": [{"document_id": 1, "is_parent": True}, {"document_id": 4, "is_parent": True}],
            "waypoints": [],
        }
        ...
        expected_to_remove = [{"document_id": 4, "is_parent": True, "doc_type": ROUTE_TYPE}]

        self.assertEqual(_get_document_ids(to_add), _get_document_ids(expected_to_add))
        self.assertEqual(_get_document_ids(to_remove), _get_document_ids(expected_to_remove))
    ...
    def test_synchronize_associations(self):
        ...
        self.session.add_all(
            [
                Association.create(parent_document=self.route1, child_document=self.route2),
                Association.create(parent_document=self.route1, child_document=self.route3),
            ]
        )
        self.session.flush()
        ...
        synchronize_associations(self.route1, new_associations, self.global_userids["contributor"])
        ...
        self.assertIsNotNone(self._get_association(self.waypoint1, self.route1))

    def _get_association(self, main_doc, child_doc):
        return self.session.query(Association).get((main_doc.document_id, child_doc.document_id))
