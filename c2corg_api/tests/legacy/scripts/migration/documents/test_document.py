...
class DocumentTest(unittest.TestCase):
    def test_extract_summary(self):
        ...
        text, summary = d.extract_summary("... [abstract][b]the abstract[/b][/abstract] abc")
        self.assertEqual(text, "...  abc")
        self.assertEqual(summary, "[b]the abstract[/b]")

        text, summary = d.extract_summary("... [abs]the abstract[/abs] abc")
        self.assertEqual(text, "...  abc")
        self.assertEqual(summary, "the abstract")

        text, summary = d.extract_summary("... [abs]the abstract\nmultiline[/abs] abc")
        self.assertEqual(text, "...  abc")
        self.assertEqual(summary, "the abstract\nmultiline")

        # if there is more than one abstract, only the first is extracted
        # (currently there is no document locale with two abstracts)
        text, summary = d.extract_summary("... [abs]the abstract[/abs] abc" "[abs]2nd abstract[/abs]")
        self.assertEqual(text, "...  abc")
        self.assertEqual(summary, "the abstract")

        text, summary = d.extract_summary("... [b]not the abstract[/b] abc")
    ...
    def test_convert_tags(self):
        ...
        text = d.convert_q_tags("... [q][b]some whatever[/b] content[/q] abc")
        self.assertEqual(text, "... [quote][b]some whatever[/b] content[/quote] abc")

        text = d.convert_c_tags("... [c][b]some whatever[/b] content[/c] abc")
        self.assertEqual(text, "... [code][b]some whatever[/b] content[/code] abc")

        text = d.convert_tags("... [[users/123456/fr|Toto le héros]] et ses copains")
        self.assertEqual(text, "... [[profiles/123456/fr|Toto le héros]] et ses copains")
