...
class TestFeedRest(BaseFeedTestRest):
    def test_get_public_feed(self):
        ...
        self.assertEqual(self.outing.document_id, latest_change["document"]["document_id"])
    ...
    def test_get_public_feed_ignoring_admin(self):
        ...
        latest_change = feed[0]
        self.assertEqual(self.waypoint2.document_id, latest_change["document"]["document_id"])
    ...
    def test_get_public_feed_pagination_invalid_format(self):
        response = self.app.get(self._prefix + "?token=123,invalid-token", status=400)
    ...
    def test_get_public_feed_pagination(self):
        ...
        # last 2 changes
        response = self.app.get(self._prefix + "?limit=2&token=" + pagination_token, status=200)
        body = response.json
...
class TestPersonalFeedRest(BaseFeedTestRest):
    ...
    def test_get_feed_activities_filter(self):
        ...
        self.assertEqual(self.route.document_id, feed[0]["document"]["document_id"])
    ...
    def test_get_feed_langs_filter(self):
        ...
        self.assertEqual(self.outing.document_id, feed[0]["document"]["document_id"])
        self.assertEqual(self.waypoint2.document_id, feed[1]["document"]["document_id"])

    def test_get_feed_areas_filter(self):
        """Get personal feed with an area filter."""
        # set an area filter for the user
        self.session.add(FilterArea(area_id=self.area1.document_id, user_id=self.global_userids["contributor"]))
        self.session.flush()
        ...
        self.assertEqual(self.route.document_id, feed[0]["document"]["document_id"])
        self.assertEqual(self.waypoint1.document_id, feed[1]["document"]["document_id"])
    ...
    def test_get_feed_followed_user_and_activity_filter(self):
        ...
        self.assertEqual(self.outing.document_id, feed[0]["document"]["document_id"])
        self.assertEqual(self.route.document_id, feed[1]["document"]["document_id"])
        self.assertEqual(self.waypoint2.document_id, feed[2]["document"]["document_id"])
    ...
    def test_get_feed_areas_filter_paginated(self):
        ...
        # first page
        headers = self.add_authorization_header(username="contributor")
        response = self.app.get("/personal-feed?limit=1", status=200, headers=headers)
        body = response.json
        ...
        # second page
        response = self.app.get("/personal-feed?limit=1&token=" + pagination_token, status=200, headers=headers)
        body = response.json

        feed = body["feed"]
        self.assertEqual(1, len(feed))
        self.assertEqual(self.waypoint1.document_id, feed[0]["document"]["document_id"])
        pagination_token = body["pagination_token"]
...
class TestProfileFeedRest(BaseFeedTestRest):
    def test_get_feed_invalid_user_id(self):
        headers = self.add_authorization_header(username="contributor")
        self.app.get("/profile-feed?u=invalid-user-id", status=400, headers=headers)

    def test_get_feed_missing_user_id(self):
        headers = self.add_authorization_header(username="contributor")
        self.app.get("/profile-feed", status=400, headers=headers)

    def test_get_feed_non_existing_user(self):
        headers = self.add_authorization_header(username="contributor")
        self.app.get("/profile-feed?u=-1", status=404, headers=headers)

    def test_get_profile_contributor(self):
        """Get profile feed for 'contributor'."""
        headers = self.add_authorization_header(username="contributor")
        user_id = self.global_userids["contributor"]
        response = self.app.get("/profile-feed?u=" + str(user_id), status=200, headers=headers)
        body = response.json
        ...
        self.assertEqual(self.outing.document_id, feed[0]["document"]["document_id"])
        self.assertEqual(self.route.document_id, feed[1]["document"]["document_id"])
        self.assertEqual(self.waypoint1.document_id, feed[2]["document"]["document_id"])
    ...
    def test_get_profile_contributor2_paginated(self):
        ...
        feed = body["feed"]
        self.assertEqual(1, len(feed))
        self.assertEqual(self.waypoint2.document_id, feed[0]["document"]["document_id"])
        pagination_token = body["pagination_token"]
