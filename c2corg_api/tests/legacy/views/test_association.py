...
pytestmark = pytest.mark.skip("Associations are handled totally differently")
import datetime
...
from c2corg_api.legacy.models.outing import Outing, OUTING_TYPE
...
from c2corg_api.legacy.models.xreport import Xreport, XreportLocale
...
class TestAssociationRest(BaseTestRest):
    ...
    def test_add_association(self):
        ...
        self.app_post_json(TestAssociationRest.prefix, request_body, headers=headers, status=200)

        association = self.session.query(Association).get((self.waypoint1.document_id, self.waypoint2.document_id))
        ...
        assert association_log.is_creation == True
    ...
    def test_add_association_wa(self):
        ...
        association = self.session.query(Association).get((self.waypoint1.document_id, self.article1.document_id))
        assert association is not None

        association_log = (
            self.session.query(AssociationLog)
            .filter(AssociationLog.parent_document_id == self.waypoint1.document_id)
            .filter(AssociationLog.child_document_id == self.article1.document_id)
            .one()
        )
        assert association_log.is_creation == True
        assert association_log.user_id is not None
    ...
    def test_add_association_uo(self):
        ...
        association = self.session.query(Association).get((contributor2, self.outing.document_id))
        assert association is not None

        # check that the feed change is updated
        feed_change = self.get_feed_change(self.outing.document_id, change_type="updated")
        assert feed_change is not None
        assert feed_change.change_type == "updated"
        self.assertEqual(
            set(feed_change.user_ids), set([self.global_userids["contributor"], self.global_userids["contributor2"]])
        )

        # check that the participants of the 2nd feed change are also updated
        feed_change = self.get_feed_change(self.outing.document_id, change_type="added_photos")
        assert feed_change is not None
        self.assertEqual(
    ...
    def test_add_association_uo_no_rights(self):
        ...
        response = self.app_post_json(TestAssociationRest.prefix, request_body, headers=headers, status=400)
        body = response.json

        self.assertError(
            body["errors"],
            "associations.outings",
            "no rights to modify associations with outing {}".format(self.outing.document_id),
        )
    ...
    def test_add_association_wc_article_personal_unauthorized(self):
        ...
        self.assertError(
            ...
            "no rights to modify associations with article {}".format(self.article1.document_id),
        )
    ...
    def test_add_association_wi_image_personal_unauthorized(self):
        ...
        self.assertError(
            body["errors"],
            "associations.images",
            "no rights to modify associations with image {}".format(self.image1.document_id),
        )
    ...
    def test_add_association_wx_xreport_unauthorized(self):
        ...
        self.assertError(
            ...
            "no rights to modify associations with xreport {}".format(self.report1.document_id),
        )
    ...
    def test_add_association_duplicate(self):
        ...
        self.app_post_json(TestAssociationRest.prefix, request_body, headers=headers, status=400)
    ...
    def test_add_association_invalid(self):
        ...
        assert errors[0].get("name") == "association"
        assert errors[0].get("description") == "invalid association type"
    ...
    def test_add_association_redirected_document(self):
        ...
        assert errors[0].get("name") == "child_document_id"
        assert errors[0].get("description") == "child document does not exist or is redirected"
    ...
    def test_add_association_invalid_ids(self):
        ...
        assert errors[0].get("name") == "parent_document_id"
        assert errors[0].get("description") == "parent document does not exist or is redirected"

        assert errors[1].get("name") == "child_document_id"
        assert errors[1].get("description") == "child document does not exist or is redirected"
    ...
    def test_delete_association_not_existing(self):
        ...
        self.app.delete_json(TestAssociationRest.prefix, request_body, headers=headers, status=400)
    ...
    def test_delete_association(self):
        ...
        self.app.delete_json(TestAssociationRest.prefix, request_body, headers=headers, status=200)
    ...
    def test_delete_association_non_moderator(self):
        ...
        response = self.app.delete_json(TestAssociationRest.prefix, request_body, headers=headers, status=400)
        body = response.json
        ...
        self.assertError(
            ...
            "w ({}) and w ({})".format(self.waypoint1.document_id, self.waypoint2.document_id),
        )
    ...
    def test_delete_association_main_waypoint(self):
        request_body = {"parent_document_id": self.waypoint2.document_id, "child_document_id": self.route1.document_id}
        headers = self.add_authorization_header(username="contributor")
        ...
        self.assertEqual(
            errors[0].get("description"), "as the main waypoint of the route, this waypoint can not " "be disassociated"
        )
    ...
    def test_delete_association_wc_article_personal(self):
        ...
        self.app_post_json(TestAssociationRest.prefix, request_body, headers=headers1, status=200)
        ...
        response = self.app.delete_json(TestAssociationRest.prefix, request_body, headers=headers2, status=400)
        body = response.json
        ...
        self.assertError(
            ...
            "Bad Request",
            "no rights to modify associations between document "
            "w ({}) and c ({})".format(self.waypoint1.document_id, self.article1.document_id),
        )

        # but the original user can delete it
        self.app.delete_json(TestAssociationRest.prefix, request_body, headers=headers1, status=200)

    def test_delete_association_wp_r_last_waypoint(self):
        request_body1 = {"parent_document_id": self.waypoint2.document_id, "child_document_id": self.route1.document_id}
        headers = self.add_authorization_header(username="moderator")

        # add association
        self.app_post_json(TestAssociationRest.prefix, request_body1, headers=headers, status=200)

        # try to delete the association
        response = self.app.delete_json(TestAssociationRest.prefix, request_body1, headers=headers, status=400)

        body = response.json
        errors = body.get("errors")
        self.assertError(
            errors, "Bad Request", "as the last waypoint of the route, this waypoint can not be " "disassociated"
        )

        # add a second waypoint
        request_body2 = {"parent_document_id": self.waypoint1.document_id, "child_document_id": self.route1.document_id}
        self.app_post_json(TestAssociationRest.prefix, request_body2, headers=headers, status=200)

        # now, the first waypoint can be unlinked
        self.app.delete_json(TestAssociationRest.prefix, request_body1, headers=headers)

    def test_delete_association_ro_last_route(self):
        request_body1 = {"parent_document_id": self.route1.document_id, "child_document_id": self.outing.document_id}
        headers = self.add_authorization_header(username="moderator")
        ...
        errors = body.get("errors")
        self.assertError(
            errors, "Bad Request", "as the last route of the outing, this route can not be " "disassociated"
        )

        # add a second route
        request_body2 = {"parent_document_id": self.route2.document_id, "child_document_id": self.outing.document_id}
        self.app_post_json(TestAssociationRest.prefix, request_body2, headers=headers, status=200)
    ...
    def _add_test_data(self):
        ...
        self.waypoint3 = Waypoint(waypoint_type="summit", elevation=200, redirects_to=self.waypoint1.document_id)
        ...
        self.image1 = Image(filename="image.jpg", locales=[DocumentLocale(lang="en", title="Mont Blanc from the air")])
        ...
        self.report1 = Xreport(
            event_activity="alpine_climbing", locales=[XreportLocale(lang="en", title="Lac d'Annecy")]
        )
        self.session_add(self.report1)
        self.session.flush()
        DocumentRest.create_new_version(self.report1, user_id)

        self.outing = Outing(
            activities=["skitouring"], date_start=datetime.date(2016, 1, 1), date_end=datetime.date(2016, 1, 1)
        )
        ...
        feed_change2.user_ids = list(set(feed_change.user_ids).union([user_id]))
        self.session_add(feed_change2)
        self.session.flush()
