...
from c2corg_api.legacy.models.outing import OutingLocale, Outing, OUTING_TYPE
...
from c2corg_api.legacy.models.image import Image, ArchiveImage, IMAGE_TYPE
...
from c2corg_api.tests.legacy.views import BaseDocumentTestRest, BaseTestRest
...
class BaseTestImage(BaseDocumentTestRest):
    def _add_test_data(self):
        ...
        self.locale_fr = DocumentLocale(lang="fr", title="Mont Blanc du ciel", description="...")
        ...
        self.session.flush()
        self._add_association(Association.create(parent_document=self.article1, child_document=self.image), user_id)

        self.book1 = Book(activities=["hiking"], book_types=["biography"])
        self.session_add(self.book1)
        self.session.flush()
        self._add_association(Association.create(parent_document=self.book1, child_document=self.image), user_id)

        DocumentRest.create_new_version(self.image, user_id)

        self.image_version = self.session_query_first(DocumentVersion, document_id=self.image.document_id)

        self.image2 = Image(filename="image2.jpg", activities=["paragliding"], height=1500)
        self.session_add(self.image2)
        self.image3 = Image(filename="image3.jpg", activities=["paragliding"], height=1500)
        self.session_add(self.image3)
        self.image4 = Image(filename="image4.jpg", activities=["paragliding"], height=1500, image_type="personal")
        self.image4.locales.append(DocumentLocale(lang="en", title="Mont Blanc from the air", description="..."))
        self.image4.locales.append(DocumentLocale(lang="fr", title="Mont Blanc du ciel", description="..."))
        self.session_add(self.image4)
        self.session.flush()
        DocumentRest.create_new_version(self.image3, self.global_userids["contributor2"])
        DocumentRest.create_new_version(self.image4, user_id)

        self._add_association(Association.create(parent_document=self.image, child_document=self.image2), user_id)
        ...
        self.area = Area(area_type="range", locales=[DocumentLocale(lang="fr", title="France")])
        self.session_add(self.area)
        self.session.flush()
        ...
        self.session.flush()
        self._add_association(Association.create(parent_document=self.outing1, child_document=self.image), user_id)
    ...
    def _post_success_document(self, overrides={}):
        doc = {
            ...
            "height": 1500,
            "geometry": {"id": 5678, "version": 6789, "geom": '{"type": "Point", "coordinates": [635956, 5723604]}'},
    ...
    def _validate_post_success(self, body, doc, check_wp=True):
        ...
        if check_wp:
            ...
            association_wp = self.session.query(Association).get((self.waypoint.document_id, doc.document_id))
...
class TestImageRest(BaseTestImage):
    def setup_method(self):
        self.set_prefix_and_model("/images", IMAGE_TYPE, Image, ArchiveImage, ArchiveDocumentLocale)
        BaseTestImage.setup_method(self)
        self._add_test_data()
        self.session.commit()
    ...
    def test_get_collection_paginated(self):
        self.get("/images?offset=invalid", status=400)

        self.assertResultsEqual(self.get_collection({"offset": 0, "limit": 0}), [], 4)

        self.assertResultsEqual(self.get_collection({"offset": 0, "limit": 1}), [self.image4.document_id], 4)
        self.assertResultsEqual(
            self.get_collection({"offset": 0, "limit": 2}), [self.image4.document_id, self.image3.document_id], 4
        )
        self.assertResultsEqual(
            self.get_collection({"offset": 1, "limit": 2}), [self.image3.document_id, self.image2.document_id], 4
        )
    ...
    def test_get_collection_search(self):
        ...
        self.assertResultsEqual(
            self.get_collection_search({"l": "en"}), [self.image4.document_id, self.image.document_id], 2
        )

        self.assertResultsEqual(
            self.get_collection_search({"act": "paragliding"}),
            [self.image4.document_id, self.image3.document_id, self.image2.document_id, self.image.document_id],
            4,
        )

    def test_get(self):
        body = self.get_custom(self.image)
        self._assert_geometry(body)
    ...
    def test_get_edit(self):
        response = self.get(self._prefix + "/" + str(self.image.document_id) + "?e=1", status=200)
        body = response.json
    ...
    @patch("c2corg_api.views.image.requests.post", return_value=Mock(status_code=500, reason="test error"))
    def test_post_image_backend_error(self, post_mock):
        headers = self.add_authorization_header(username="contributor")
        r = self.app_post_json(self._prefix, self._post_success_document(), headers=headers, status=500)
        assert "test error" in r.text

    @patch("c2corg_api.views.image.requests.post", return_value=Mock(status_code=200))
    def test_post_success(self, post_mock):
        waypoint_cache_key = self.session.query(CacheVersion).get(self.waypoint.document_id).version
    ...
    def test_post_no_permission_for_outing_association(self, post_mock):
        ...
        response = self.app_post_json(self._prefix, body_post, headers=headers, expect_errors=True, status=400)

        body = response.json
        self.assertError(
            body.get("errors"),
            "Bad Request",
            "no rights to modify associations with outing {}".format(self.outing1.document_id),
        )
    ...
    def test_put_wrong_locale_version(self):
        body = {
            "document": {
                ...
                "image_type": "collaborative",
                "height": 1500,
                "locales": [{"lang": "en", "title": "Mont Blanc from the air", "description": "...", "version": -9999}],
            }
        }
        self.put_wrong_version(body, self.image.document_id)
    ...
    def test_put_wrong_user(self):
        ...
        headers = self.add_authorization_header(username="contributor2")
        self.app_put_json(self._prefix + "/" + str(self.image4.document_id), body, headers=headers, status=403)
    ...
    def test_put_good_user(self):
        ...
        body = {
            ...
            "document": {
                ...
                "height": 2000,
                "locales": [
                    {"lang": "en", "description": "A nice picture", "version": self.image4.get_locale("en").version}
                ],
            },
        }
        headers = self.add_authorization_header(username="contributor")
    ...
    def test_put_success_all(self):
        ...
        # check that a link to the linked wp is created
        association_wp = self.session.query(Association).get((self.waypoint.document_id, image.document_id))
        assert association_wp is not None
    ...
    def test_put_no_permission_for_outing_association_removal(self):
        ...
        response = self.app_put_json(
            self._prefix + "/" + str(self.image.document_id), body, headers=headers, expect_errors=True, status=400
        )
        ...
        self.assertError(
            ...
            "document o ({}) and i ({})".format(self.outing1.document_id, self.image.document_id),
        )
    ...
    def test_put_success_new_lang(self):
        ...
        body = {
            ...
            "document": {
                ...
                "image_type": "collaborative",
                "height": 1500,
                "locales": [{"lang": "es", "title": "Mont Blanc del cielo", "description": "..."}],
            },
        }
    ...
    def test_change_image_type_collaborative(self):
        ...
        headers = self.add_authorization_header(username="contributor")
        self.app_put_json(self._prefix + "/" + str(self.image.document_id), body, headers=headers, status=400)
    ...
    def test_change_image_type_collaborative_moderator(self):
        ...
        self.app_put_json(self._prefix + "/" + str(self.image.document_id), body, headers=headers, status=200)
...
class TestImageListRest(BaseTestImage):
    ...
    def post_success(self, request_body, user="contributor"):
        ...
        response = self.app_post_json("/images/list", request_body, headers=headers, status=200)
    ...
    def test_post_multiple(self, post_mock):
        body = {
            "images": [
                self._post_success_document({"filename": "post_image2.jpg", "locales": [{"lang": "en"}]}),
    ...
    def test_post_multiple_for_outing(self, post_mock):
        ...
        association_img = self.session.query(Association).get((self.outing1.document_id, doc.document_id))
    ...
    def test_post_multiple_as_contributor2(self, post_mock):
        ...
        feed_change = self.get_feed_change(self.waypoint.document_id, change_type="added_photos")
        assert feed_change is not None
        ...
        self.session_refresh(feed_change)
        assert feed_change.image3_id is not None
        assert feed_change.more_images is True

    @patch("c2corg_api.views.image.requests.post", return_value=Mock(status_code=200))
    def test_post_validation_error(self, post_mock):
        request_body = {
            "images": [
                self._post_success_document({"geometry": {"geom": '{"coordinates": [1, null], "type": "Point"}'}})
            ]
        }

        headers = self.add_authorization_header(username="contributor")
        response = self.app_post_json("/images/list", request_body, headers=headers, status=400)
...
class TestImageProxyRest(BaseTestRest):
    ...
    def _add_test_data(self):
        self.image = Image(filename="image.jpg", activities=["paragliding"], height=1500, image_type="collaborative")
        self.session_add(self.image)
        self.image2 = Image(filename="image.svg", activities=["paragliding"], height=1500, image_type="collaborative")
        self.session_add(self.image2)
        self.session.flush()

    def test_get_not_exists(self):
        self.get("/images/proxy/{}".format(999), status=404)

    def test_bad_size(self):
        resp = self.get("/images/proxy/{}?size=badsize".format(self.image.document_id), status=400)
        errors = resp.json.get("errors")
        assert "invalid size" == errors[0].get("description")

    def test_success_without_size(self):
        resp = self.get("/images/proxy/{}".format(self.image.document_id), status=302)
        assert "image.jpg" in resp.headers["Location"]

    def test_success_with_size(self):
        resp = self.get("/images/proxy/{}?size=BI".format(self.image.document_id), status=302)
        assert "imageBI.jpg" in resp.headers["Location"]

    def test_svg_without_size(self):
        resp = self.get("/images/proxy/{}".format(self.image2.document_id), status=302)
        assert "image.svg" in resp.headers["Location"]

    def test_svg_with_size(self):
        resp = self.get("/images/proxy/{}?size=BI".format(self.image2.document_id), status=302)
        assert "imageBI.jpg" in resp.headers["Location"]

    def test_bad_extension(self):
        resp = self.app.get(
            "/images/proxy/{}?size=BI&extension=badextension".format(self.image.document_id), status=400
        )
        errors = resp.json.get("errors")
        assert "invalid extension" == errors[0].get("description")

    def test_format_without_size(self):
        resp = self.get("/images/proxy/{}?extension=webp".format(self.image.document_id), status=400)
        errors = resp.json.get("errors")
        assert "invalid extension" == errors[0].get("description")

    def test_format_with_size(self):
        resp = self.get("/images/proxy/{}?size=BI&extension=avif".format(self.image.document_id), status=302)
