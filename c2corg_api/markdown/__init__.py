...
def _get_cleaner():
    ...
    if not _cleaner:
        ...
        allowed_extra_attributes = {
            "a": ["c2c:role", "c2c:document-type", "c2c:document-id", "c2c:lang", "c2c:slug", "c2c:anchor"],
            "h1": ["id", "c2c:role"],
            "h2": ["id", "c2c:role"],
            "h3": ["id", "c2c:role"],
            "h4": ["id", "c2c:role"],
            ...
            "img": ["alt", "c2c:document-id", "c2c:role", "c2c:size", "c2c:url-proxy", "c2c:svg-name", "c2c:emoji-db"],
        }
...
def _get_markdown_parser():
    ...
    if not _markdown_parser:
        ...
        _markdown_parser = markdown.Markdown(output_format="xhtml5", extensions=extensions)
